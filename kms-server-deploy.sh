...
auto_start() {
	if [[ x"${release}" == x"centos" ]]; then
        ...
        if ! wget --no-check-certificate -O /etc/init.d/kms https://raw.githubusercontent.com/nswl01/kms-server/main/kms-server-deploy.sh/kms-debian; then
            echo -e "${Error} Failed to download KMS Server script."
            exit 1
        fi
    else
...
start_install() {
	...
	make
	if [ $? -ne 0 ]; then
        echo -e "${Error} 编译KMS 服务端出错,请重试或者将出错信息提交到 https://github.com/nswl01/kms-server/main/kms-server-deploy.sh/issues."
        exit 1
    fi
	auto_start
	mkdir /usr/local/kms
...
echo -e "  vlmcsd服务端 一键管理脚本 ${Red_font_prefix}Powered By qylx.uno${Font_color_suffix}
